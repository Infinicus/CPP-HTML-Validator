...
void HTMLIsValid(stack<string> string_stack)
{
    if(string_stack.empty()) cout << "\nDocument is valid!" << endl;              //Validation
                else cout << "\nDocument is not valid!" << endl;
}

int main()
{
    ifstream htmlfile("index.html");                                            //input html file
    ...
    while(getline(htmlfile, line))                                              //Loops through the file
    {
        ...
        {                                                                       //line. (Max 100 tags)
            int n = 0;
            pos_open = line.find('<', pos_open + tags_counter);                 //Position of the tags with + counter (pos + 1 is the second occurence, and so on)
            ...
            if(pos_open != string::npos)                                        //check for open tag (<), if tag is found, execute.
            {
                ...
                {
                    //printHTML(line[i]);                                       ////Function to print HTML as it is (for checking)
    ...
    HTMLIsValid(tags_stack);                                                    //Final validation.
